...
const events: Event[] = [
  { id: 1, title: "Passeio Botânico no Jardim Gulbenkian", theme: "Natureza", date: addDays(baseDate, 0), time: "10:00", region: "Lisboa", location: "Jardim Gulbenkian, Lisboa", spots: 12, description: "Exploração guiada de espécies vegetais com atividades de observação e registo científico." },
  { id: 2, title: "Oficina de Robótica com Reciclados", theme: "Construção", date: addDays(baseDate, 1), time: "14:30", region: "Porto", location: "Fábrica de Ciência, Porto", spots: 8, description: "Construção de robots com materiais reciclados. Introdução a circuitos simples." },
  { id: 3, title: "Laboratório de Cores Naturais", theme: "Arte", date: addDays(baseDate, 2), time: "10:00", region: "Centro", location: "Mata do Buçaco, Mealhada", spots: 15, description: "Criar tintas e pigmentos a partir de elementos naturais. Pintura ao ar livre." },
  { id: 4, title: "Experiências com Água", theme: "Ciência", date: addDays(baseDate, 3), time: "15:00", region: "Algarve", location: "Praia da Rocha, Portimão", spots: 10, description: "Experiências científicas sobre flutuação, densidade e ciclo da água junto ao mar." },
  { id: 5, title: "Clube de Leitura ao Ar Livre", theme: "Leitura", date: addDays(baseDate, 4), time: "11:00", region: "Alentejo", location: "Jardim Público, Évora", spots: 20, description: "Sessão de leitura partilhada com atividades de escrita criativa para todas as idades." },
  { id: 6, title: "Construção de Abrigos na Floresta", theme: "Natureza", date: addDays(baseDate, 5), time: "09:30", region: "Norte", location: "Parque Peneda-Gerês", spots: 10, description: "Aprender técnicas de construção de abrigos usando apenas materiais naturais." },
  { id: 7, title: "Pintura Mural Colaborativa", theme: "Arte", date: addDays(baseDate, 6), time: "14:00", region: "Lisboa", location: "LX Factory, Lisboa", spots: 16, description: "Criação coletiva de um mural inspirado na biodiversidade portuguesa." },
  { id: 8, title: "Foguetões de Papel", theme: "Ciência", date: addDays(baseDate, 1), time: "10:30", region: "Centro", location: "Exploratório, Coimbra", spots: 12, description: "Construir e lançar foguetões de papel. Introdução à aerodinâmica." },
  { id: 9, title: "Tecelagem com Fibras Naturais", theme: "Arte", date: addDays(baseDate, 3), time: "10:00", region: "Norte", location: "Casa das Artes, Braga", spots: 8, description: "Oficina de tecelagem usando lã, algodão e fibras vegetais." },
];
...
const CalendarPage = () => {
  ...
  const currentWeekStart = addDays(baseDate, weekOffset * 7);
  const weekDays = Array.from({ length: 7 }, (_, i) => addDays(currentWeekStart, i));

  const filteredByTheme = themeFilter === "all" ? events : events.filter((e) => e.theme === themeFilter);

  const filteredByRegion = selectedRegion
    ? filteredByTheme.filter((e) => e.region === selectedRegion)
    : filteredByTheme;

  const showDetails = (ev: Event) => {
    toast({
      title: ev.title,
      description: `${ev.location} · ${ev.time} · ${ev.spots} vagas`,
    });
  };
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground">Eventos, encontros e atividades por todo o país.</p>
          </div>
              ...
              {weekDays.map((day) => {
                ...
                return (
                  <div key={day.toISOString()} className="min-h-[140px]">
                    <div
                      className={`text-center text-xs font-semibold mb-2 pb-1 border-b ${
                  ...
                  <div className="relative w-full max-w-[260px] mx-auto" style={{ aspectRatio: "0.5" }}>
                    ...
                    {regions.map((region) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`h-8 w-8 rounded-full flex items-center justify-center text-xs font-bold shadow-soft transition-colors ${
                              isSelected
                                ? "gradient-warmth text-primary-foreground"
